...
export function EngagementPrompt() {
  ...
  const actionLabel = prompt?.kind === 'predict' ? 'Predict now' : 'View table'
  const emoji = prompt?.kind === 'predict' ? '🎯' : '🏆'
          ...
          <div className="mx-auto max-w-lg overflow-hidden rounded-2xl border border-simelabs/35 bg-card shadow-[0_8px_32px_rgb(0_0_0/0.25),0_0_24px_rgb(38_203_153/0.15)]">
            <div className="flex items-start gap-3 p-4">
              {prompt.kind === 'predict' ? (
                ...
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-simelabs/15 text-xl">
                  {emoji}
                </div>
              )}
            ...
            {prompt.kind === 'predict' && prompt.urgent && (
              <motion.div
                className="h-0.5 bg-gradient-to-r from-simelabs via-simelabs-light to-simelabs"
